...
namespace v8 {
    namespace internal {
        namespace regexp {
            ...
            static constexpr int kMaxSingleCharValue = OperandTypeTraits<BytecodeOperandType::kChar>::kMaxValue;
            ...
            void BytecodeWriter::EmitRawBytecodeStream(const BytecodeWriter* src_writer, int src_offset, int length) {
                ...
                while (jump_iter != src_edges.end() && jump_iter->first < src_offset + length) {
            ...
            template <typename T>
            void BytecodeWriter::EmitOperand(BytecodeOperandType type, T value, int offset) {
                switch (type) {
                    #define CASE(Name, ...)              \
            ...
            template <>
            void BytecodeWriter::EmitOperand<ReBcOpType::kJumpTarget>(Label* label, int offset) {
            ...
            template <>
            void BytecodeWriter::EmitOperand<ReBcOpType::kBitTable>(Handle<ByteArray> table, int offset) {
            ...
            template <>
            void BytecodeWriter::EmitOperand<ReBcOpType::kBitTable>(const uint8_t* src, int offset) {
                ...
                static constexpr int kWordCount = (RegExpMacroAssembler::kTableSize / (kBitsPerByte * kInt32Size));
            ...
            void BytecodeGenerator::PushRegister(int register_index, StackCheckFlag check_stack_limit) {
                Emit<Bytecode::kPushRegister>(register_index, check_stack_limit);
            }

            void BytecodeGenerator::WriteCurrentPositionToRegister(int register_index, int cp_offset) {
            ...
            void BytecodeGenerator::CheckSpecialClassRanges(StandardCharacterSet type, Label* on_no_match) {
                DCHECK(CanOptimizeSpecialClassRanges(type));
                Emit<Bytecode::kCheckSpecialClassRanges>(type, on_no_match);
            }

            void BytecodeGenerator::LoadCurrentCharacterImpl(int cp_offset, Label* on_failure, bool check_bounds, int characters, int bounds_check_offset) {
                CHECK(base::IsInRange(cp_offset, kMinCPOffset, kMaxCPOffset));
                if (check_bounds) {
                    DCHECK_LE(cp_offset, bounds_check_offset);
                    if (characters == 4) {
                        Emit<Bytecode::kLoad4CurrentChars>(cp_offset, bounds_check_offset, on_failure);
                    } else if (characters == 2) {
                        Emit<Bytecode::kLoad2CurrentChars>(cp_offset, bounds_check_offset, on_failure);
                    } else {
                        DCHECK_EQ(1, characters);
                        Emit<Bytecode::kLoadCurrentCharacter>(cp_offset, bounds_check_offset, on_failure);
                    }
                } else {
                    if (characters == 4) {
            ...
            void BytecodeGenerator::CheckCharacterAfterAnd(uint32_t c, uint32_t mask, Label* on_equal) {
            ...
            void BytecodeGenerator::CheckNotCharacterAfterAnd(uint32_t c, uint32_t mask, Label* on_not_equal) {
                // TODO(pthier): This is super hacky. We could still check for 4 characters
                // (with the last 2 being 0 after masking them), but not emit AndCheck4Chars.
                // This is rather confusing and should be changed.
                if (c > kMaxSingleCharValue) {
            ...
            void BytecodeGenerator::CheckNotCharacterAfterMinusAnd(base::uc16 c, base::uc16 minus, base::uc16 mask, Label* on_not_equal) {
                Emit<Bytecode::kCheckNotCharacterAfterMinusAnd>(c, minus, mask, on_not_equal);
            }

            void BytecodeGenerator::CheckCharacterInRange(base::uc16 from, base::uc16 to, Label* on_in_range) {
                Emit<Bytecode::kCheckCharacterInRange>(from, to, on_in_range);
            }

            void BytecodeGenerator::CheckCharacterNotInRange(base::uc16 from, base::uc16 to, Label* on_not_in_range) {
                Emit<Bytecode::kCheckCharacterNotInRange>(from, to, on_not_in_range);
            }

            void BytecodeGenerator::CheckBitInTable(Handle<ByteArray> table, Label* on_bit_set) {
            ...
            void BytecodeGenerator::SkipUntilCharAnd(
                int cp_offset,
                int advance_by,
                unsigned character,
                unsigned mask,
            ...
            void BytecodeGenerator::SkipUntilCharOrChar(
                int cp_offset,
                int advance_by,
                unsigned char1,
                unsigned char2,
            ...
            void BytecodeGenerator::SkipUntilOneOfMasked3(const SkipUntilOneOfMasked3Args& args) {
            ...
            void BytecodeGenerator::CheckNotBackReference(int start_reg, bool read_backward, Label* on_not_equal) {
                if (read_backward) {
            ...
            void BytecodeGenerator::CheckNotBackReferenceIgnoreCase(int start_reg, bool read_backward, bool unicode, Label* on_not_equal) {
                if (read_backward) {
                    if (unicode) {
            ...
            // void BytecodeGenerator::CheckNotBackReferenceIgnoreCase(int start_reg, bool read_backward, bool unicode, Label* on_not_equal) {
            void BytecodeGenerator::CheckNotBackReferenceIgnoreCase(
                int start_reg,
                bool read_backward,
                bool unicode,
                Label* on_not_equal
                ...
                if (read_backward) {
                    if (unicode) {
                        Emit<Bytecode::kCheckNotBackRefNoCaseUnicodeBackward>(start_reg, on_not_equal);
                    } else {
                        Emit<Bytecode::kCheckNotBackRefNoCaseBackward>(start_reg, on_not_equal);
                    }
                } else {
            ...
            void BytecodeGenerator::IfRegisterLT(int register_index, int comparand, Label* on_less_than) {
                Emit<Bytecode::kIfRegisterLT>(register_index, comparand, on_less_than);
            }

            void BytecodeGenerator::IfRegisterGE(int register_index, int comparand, Label* on_greater_or_equal) {
            ...
            DirectHandle<HeapObject> BytecodeGenerator::GetCode(DirectHandle<RegExpData> re_data, Flags flags) {
                Bind(&backtrack_);
                Backtrack();

                DirectHandle<TrustedByteArray> array;
                if (v8_flags.regexp_peephole_optimization) {
                    array = BytecodePeepholeOptimization::OptimizeBytecode(isolate_, zone(), re_data, this);
                } else {
                    array = isolate_->factory()->NewTrustedByteArray(static_cast<uint32_t>(pc_));
            ...
            #define CASE(Name, ...)                                      \
                ...
                BytecodeWriter::EmitOperand<BytecodeOperandType::k##Name>(OperandTypeTraits<BytecodeOperandType::k##Name>::kCType, int);
